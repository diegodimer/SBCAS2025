...
def remove_instances_2(x, conditions: list, percentage: float):
    ...
    drop_indices = np.random.choice(
        new_x.index, min(
            percentage, new_x_size) if percentage >= 1 else min(
            round(
                new_x_size * percentage), new_x_size), replace=False)
...
def high_imbalance():
    ...
    def perturbe(X_train, y_train):
        ...
        new_x_train = remove_instances_2(
            new_x_train, [
                new_x_train['Sex'] == 0, new_x_train['Diabetes_binary'] == 0], .85)
        new_x_train = remove_instances_2(
            new_x_train, [
                new_x_train['Sex'] == 1, new_x_train['Diabetes_binary'] == 1], 0.85)

        new_x_train = remove_instances_2(
            new_x_train, [
                new_x_train['Age'] != 1, new_x_train['Diabetes_binary'] == 0], .55)
        new_x_train = remove_instances_2(
            new_x_train, [
                new_x_train['Age'] == 1, new_x_train['Diabetes_binary'] == 1], .45)

        new_x_train = remove_instances_2(new_x_train,
                                         [new_x_train['HighBP'] == 0,
                                          new_x_train['Diabetes_binary'] == 0,
                                             new_x_train['Sex'] == 0],
                                         0.2)
        new_x_train = remove_instances_2(new_x_train,
                                         [new_x_train['HighBP'] == 1,
                                          new_x_train['Diabetes_binary'] == 1,
                                             new_x_train['Sex'] == 1],
                                         0.2)
    ...
    print("Remove 85% of women with negative output and 85% of men with positive output, respectively\n"
...
def equal_balance():
    ...
    def perturbe(X_train, y_train):
        ...
        min_count = min(
            len(privileged_highbp_male),
            len(privileged_highbp_female),
        ...
        balanced_x_train = pd.concat(
            [
                balanced_privileged_highbp_male,
                balanced_privileged_highbp_female,
                balanced_unprivileged_highbp_male,
                balanced_unprivileged_highbp_female,
                balanced_privileged_lowbp_male,
                balanced_privileged_lowbp_female,
                balanced_unprivileged_lowbp_male,
                balanced_unprivileged_lowbp_female]).reset_index(
            drop=True)

        global train_size_eq
        train_size_eq += len(balanced_x_train)
        balanced_x_train = balanced_x_train.drop('index', axis=1)
        return balanced_x_train.drop(
            h.predicted_attr, axis=1), balanced_x_train[h.predicted_attr]
