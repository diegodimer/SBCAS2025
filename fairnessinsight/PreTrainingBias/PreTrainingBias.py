...
class PreTrainingBias:
    ...
    #     total_accepted = len(df[df[target] == positive_outcome])
    #     total_rejected = len(df[df[target] != positive_outcome])
    #     weighted_dds = []

    #     for subgroup_value in df[subgroup_attribute].unique():
    #         subgroup_df = df[df[subgroup_attribute] == subgroup_value]

    #         subgroup_observations = len(subgroup_df)
    #         # diference from the original formula: the formula uses n(0), which would be the number of observations in the
    #         # entire dataset, but we use the number of observations in the subgroup, which matches the formula description

    #         accepted_proportion = len(subgroup_df[subgroup_df[target] == positive_outcome]) / total_accepted
    #         rejected_proportion = len(subgroup_df[subgroup_df[target] != positive_outcome]) / total_rejected

    #         demographic_disparity = rejected_proportion - accepted_proportion

    #         weighted_dd = subgroup_observations * demographic_disparity
    #         weighted_dds.append(weighted_dd)

    #     # Calculate CDD
    #     cdd = sum(weighted_dds) / total_observations
    ...
    def dd(
            ...
            target,
            positive_outcome):
        n_facet_rejected = len(df[(df[protected_attribute] != privileged_group) & (
            df[target] != positive_outcome)])
        n_facet_accepted = len(df[(df[protected_attribute] != privileged_group) & (
            df[target] == positive_outcome)])

        n_total_rejected = len(df[df[target] != positive_outcome])
        n_total_accepted = len(df[df[target] == positive_outcome])

        if n_total_rejected == 0 or n_total_accepted == 0:
            return 0.0

        dd = (n_facet_rejected / n_total_rejected) - \
            (n_facet_accepted / n_total_accepted)
        return dd
    ...
    def global_evaluation(
        ...
        dic = {
            ...
            #     group_variable,
            # ),
            f"Demographic Disparity ({protected_attribute})": self.dd(
                df, protected_attribute, privileged_group, target, positive_outcome
            )
        }
        return dic
    ...
    def pdfs_aligned_nonzero(self, *args) -> list[np.ndarray]:
        ...
        # ADDED BY ME: if key is not in the dict_pdfs, it should be with 0
        # probability
        for i in all_keys:
            for j in dict_pdfs:
                if i not in j:
        ...
        # OTHER CHANGE BY ME: if key is not present, it should have 0
        # probability. KS results would not be precise if not
