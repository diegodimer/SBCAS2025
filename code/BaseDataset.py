...
class BaseDataset:
    ...
    def _gen_pp_report(self):
        ...
        if not my_file.exists():
            Path(f"{type(self).__name__}").mkdir(parents=True, exist_ok=True)
            pp.ProfileReport(self.dataset).to_file(f"{type(self).__name__}/report.html")
    ...
    def gen_graph(
        ...
        for attr in protected_attr:
            ...
            if fig is not None:
                fig.savefig(
                    f"{type(self).__name__}/{df_type}-{predicted_attr}-{attr}.png".replace(
                        ">", ""
                    )
                ) if file_name is None else fig.savefig(
                    f"{type(self).__name__}/{file_name}.png".replace(">", "")
                )
                plt.close(fig)
